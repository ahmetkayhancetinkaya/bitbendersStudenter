...
export function SignOut(){const s=useStore();return <button className="button secondary" onClick={async()=>{try{if(supabase){const {error}=await supabase.auth.signOut();if(error)throw error}}catch(e){s.notify(errorText(e))}}}><LogOut size={17}/>Çıkış yap</button>}
